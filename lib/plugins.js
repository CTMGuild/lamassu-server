...
function consolidateTrades() {
  ...
  // #3: Consolidate only current currency
  var consolidatedTrade = {
    currency: deviceCurrency,
    satoshis: tradesQueue.filter(function(active){
        return active.currency == deviceCurrency;
      }).reduce(function(prev, current) {
      return prev + current.satoshis;
    }, 0)
  };

  // #3: Clear consolidated.
  tradesQueue = _.filter(function(active){
    return active.currency != deviceCurrency;
  });
